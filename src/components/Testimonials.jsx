...
/**
 * tableau avec 3 elements (vide, pour faker le contenu)
 * correspond aux tableau de 3 objets de l'objet JSON `testimonials`
 */
const testimonials = [
  [
    {
...
export function Testimonials() {
  ...
  /**
   * importer un tableau de traduction sous la forme suivante
   */
  const sections = t('testimonials', { returnObjects: true });
  ...
  return (
    ...
    >
      <Container>
        ...
        >
          {testimonials && testimonials.map((column, columnIndex) => (
            <li key={columnIndex}>
                      ...
                      <figcaption className="relative flex items-center justify-between pt-6 mt-6 border-t border-slate-100">
                        <div>
                          <div className="text-base font-display text-slate-900">
                            {sections[columnIndex].author.name}
                          </div>
                          <div className="mt-1 text-sm text-slate-500">
                            {sections[columnIndex].author.role}
                          </div>
                        </div>
                        <div className="overflow-hidden rounded-full bg-slate-50">
                          <Image
                            className="object-cover h-14 w-14"
                            src={sections[columnIndex].author.image}
                            alt=""
                            width={56}
                            height={56}
                          />
